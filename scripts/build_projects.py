...
def render_card(project: dict) -> list[str]:
    """Render a current GitHub repository project with the existing editorial card format."""
...
def render() -> str:
    ...
    for project in projects:
        ...
        if not str(project.get("source_repository") or "").strip():
            raise RuntimeError(f"Current project {slug} must reference a GitHub repository")
