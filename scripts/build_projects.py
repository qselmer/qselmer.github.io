...
def render() -> str:
    ...
    for project in projects:
        ...
        if not str(project.get("source_repository") or "").strip():
            raise RuntimeError(f"Project {slug} must reference a GitHub repository")
        if not str(project.get("stage") or "").strip():
            raise RuntimeError(f"Project {slug} must declare its current stage")
    ...
    for section in sections:
        ...
        if group:
            lines += ["### Projects and research directions", "", "```{=html}", f'<div class="{" ".join(class_tokens)}">']
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Research themes, staged project cards, and outputs are synchronized.")
        return
